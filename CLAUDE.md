...
- **금통위 일정은 하드코딩** — BOK 통계공표일정 페이지에 금통위는 없음(매년 10월 말 보도자료 첨부 PDF에만 게시). `sync-kr-data.mjs`의 `BOK_MPC_MEETINGS` 배열이 소스이고, 예약 루틴 `korea-econ-cal-bok-mpc-annual`(11/5·12/5)이 연 1회 갱신해 bok.ics로 방출.
- **ForexFactory 검증 status 어휘** — 'ok'/'no-comparable-events'(조용한 주, 정상)/'mismatch'(커밋 게이트 차단)/'blocked'(피드 장애·fed.ics 누락). FF는 미 지표 임팩트를 주별 재평가하므로(2026-09-21주 High USD 0건) `verify-forexfactory.mjs`는 USD High+Medium을 검증 대상으로 삼음.
